...
func (s *SettingsService) SavePrompt(index int, name, prompt, llmLabel, icon string, timeoutSec int, displayMode string, vision bool) string {
	guiLog("[GUI] JS called: SavePrompt(idx=%d, name=%s, icon=%s, timeout=%ds, mode=%s, vision=%v)", index, name, icon, timeoutSec, displayMode, vision)
	if name == "" || prompt == "" {
		return "error: name and prompt are required"
	}
	timeoutMs := timeoutSec * 1000
	entry := config.PromptEntry{Name: name, Prompt: prompt, LLM: llmLabel, Icon: icon, TimeoutMs: timeoutMs, DisplayMode: displayMode, Vision: vision}
...
func (s *SettingsService) SetIndicatorPosition(pos string) string {
	...
	// Clear saved drag position so the corner preset takes effect.
	s.cfgCopy.IndicatorX = 0
	s.cfgCopy.IndicatorY = 0
