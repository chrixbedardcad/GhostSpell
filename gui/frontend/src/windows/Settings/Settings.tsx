...
export function SettingsWindow() {
  ...
  return (
    <div className="h-full flex flex-col bg-base">
      {/* Title bar — MUST be at the very top for Wails drag to work */}
      <div
        className="flex items-center justify-between px-4 py-2 shrink-0 bg-crust border-b border-surface-0/40"
        style={{ ["--wails-draggable" as string]: "drag" }}
      >
        <div className="flex items-center gap-2">
          <img src="/dist/ghost-icon.png" alt="" className="w-5 h-5 opacity-80" />
          <span className="text-[12px] font-semibold text-subtext-1">GhostSpell Settings</span>
          {version && <span className="text-[10px] text-overlay-0">v{version}</span>}
        </div>
        <div className="flex items-center gap-3" style={{ ["--wails-draggable" as string]: "no-drag" }}>
          {/* Status indicators */}
          <div className="flex items-center gap-2 text-[10px] text-overlay-0">
            {defaultModel && (
              <span className="flex items-center gap-1" title={"LLM: " + defaultModel}>
                <span className="w-1.5 h-1.5 rounded-full bg-accent-green" />
                {defaultModel}
              </span>
            )}
            {voiceModel && (
              <span className="flex items-center gap-1" title={"Voice: " + voiceModel}>
                <span className="w-1.5 h-1.5 rounded-full bg-accent-blue" />
                {voiceModel}
              </span>
            )}
            {gpuOn && (
              <span className="flex items-center gap-1" title="GPU enabled">
          ...
          <button
            ...
            className="text-overlay-0 hover:text-accent-red px-1.5 py-0.5
                       hover:bg-surface-0/50 rounded transition-colors text-sm leading-none"
          >
            \u2715
          </button>
        </div>
      ...
      <div className="flex-1 flex min-h-0">
        {/* Sidebar */}
        <div className="w-[170px] shrink-0 bg-crust border-r border-surface-0/40 overflow-y-auto py-3 px-2">
