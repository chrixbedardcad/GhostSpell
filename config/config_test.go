...
func TestLoadValidConfig(t *testing.T) {
	...
	if len(loaded.Prompts) != 4 {
		t.Fatalf("expected 4 prompts, got %d", len(loaded.Prompts))
	}
...
func TestWriteDefault(t *testing.T) {
	...
	if len(loaded.Prompts) != 10 {
		t.Errorf("expected 10 prompts in written file, got %d", len(loaded.Prompts))
	}
}
...
func TestMigrateOldFormat(t *testing.T) {
	...
	if len(loaded.Prompts) != 6 {
		t.Fatalf("expected 6 migrated prompts, got %d", len(loaded.Prompts))
	}
	if loaded.Prompts[0].Name != "Correct" {
...
func TestNewFormatLoadsDirect(t *testing.T) {
	...
	if len(loaded.Prompts) != 5 {
		t.Fatalf("expected 5 prompts, got %d", len(loaded.Prompts))
	}
	if loaded.Prompts[0].Name != "Correct" {
