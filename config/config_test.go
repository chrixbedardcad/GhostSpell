...
func TestDefaultConfig(t *testing.T) {
	...
	if cfg.Hotkeys.Action != defaultActionHotkey {
		t.Errorf("expected default action hotkey '%s', got '%s'", defaultActionHotkey, cfg.Hotkeys.Action)
	}
...
func TestLoadValidConfig(t *testing.T) {
	...
	if len(loaded.Prompts) != 6 {
		t.Fatalf("expected 6 prompts, got %d", len(loaded.Prompts))
	}
...
func TestLoadOptionalHotkeysEmpty(t *testing.T) {
	...
	if loaded.Hotkeys.Action != defaultActionHotkey {
		t.Errorf("expected action hotkey '%s', got '%s'", defaultActionHotkey, loaded.Hotkeys.Action)
	}
...
func TestWriteDefault(t *testing.T) {
	...
	if loaded.Hotkeys.Action != defaultActionHotkey {
		t.Errorf("expected hotkey '%s' in written file, got '%s'", defaultActionHotkey, loaded.Hotkeys.Action)
	}
...
func TestMigrateOldFormat(t *testing.T) {
	...
	// Should have migrated: correction prompt + 2 rewrite templates + Define + 2 vision = 6 prompts
	if len(loaded.Prompts) != 8 {
		t.Fatalf("expected 8 migrated prompts, got %d", len(loaded.Prompts))
	}
	if loaded.Prompts[0].Name != "Correct" {
...
func TestNewFormatLoadsDirect(t *testing.T) {
	...
	if len(loaded.Prompts) != 7 {
		t.Fatalf("expected 7 prompts, got %d", len(loaded.Prompts))
	}
	if loaded.Prompts[0].Name != "Correct" {
